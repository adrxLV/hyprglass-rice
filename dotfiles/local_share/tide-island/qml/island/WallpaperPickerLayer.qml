...
FocusScope {
    ...
    readonly property string scanScript: "import hashlib,json,os,subprocess,sys\n"
        + "cache_dir=sys.argv[1]\n"
        + "wallpaper_dir=os.path.expanduser(sys.argv[2])\n"
        + "tw,th,quality=sys.argv[3],sys.argv[4],sys.argv[5]\n"
        + "exts={'.jpg','.jpeg','.png','.webp','.gif','.avif','.tiff','.bmp','.mp4','.webm','.mkv','.avi','.mov','.flv','.m4v','.wmv'}\n"
        + "vid_exts={'.mp4','.webm','.mkv','.avi','.mov','.flv','.m4v','.wmv'}\n"
        ...
        + "    if ext in vid_exts and not os.path.isfile(cache_path):\n"
        + "        try:\n"
        + "            subprocess.run(['ffmpeg','-ss','00:00:01','-i',path,'-vframes','1','-s','{}x{}'.format(tw,th),cache_path,'-y'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=3)\n"
        + "        except Exception:\n"
        + "            pass\n"
