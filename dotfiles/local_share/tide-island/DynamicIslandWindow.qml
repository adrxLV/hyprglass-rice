...
PanelWindow {
    ...
    WlrLayershell.layer: islandContainer.wallpaperPickerLayerVisible
        ...
        || islandContainer.toolsMenuLayerVisible
        || islandContainer.clipboardLayerVisible
        || islandContainer.calculatorLayerVisible
        || islandContainer.agentTextLayerVisible
        || islandContainer.agentVoiceLayerVisible
        ? WlrLayer.Overlay
        : WlrLayer.Top
    ...
    readonly property string iconFontFamily: (userConfig.iconFontFamily && userConfig.iconFontFamily !== "") ? userConfig.iconFontFamily : "JetBrainsMono Nerd Font"
    readonly property string textFontFamily: (userConfig.textFontFamily && userConfig.textFontFamily !== "") ? userConfig.textFontFamily : "SF Pro Text, SF Pro Display, SF Pro, sans-serif"
    readonly property string heroFontFamily: (userConfig.heroFontFamily && userConfig.heroFontFamily !== "") ? userConfig.heroFontFamily : "SF Pro Display, SF Pro Text, SF Pro, sans-serif"
    readonly property string timeFontFamily: (userConfig.timeFontFamily && userConfig.timeFontFamily !== "") ? userConfig.timeFontFamily : "SF Pro Display, SF Pro Text, SF Pro, sans-serif"
    ...
    function focusPowerMenu() {
        if (powerMenuLoader.item && powerMenuLoader.item.grabKeyboardFocus)
            powerMenuLoader.item.grabKeyboardFocus();
    }

    function toggleToolsMenuWindow() {
        if (islandContainer.islandState === "tools_menu")
            islandContainer.smartRestoreState();
        else
            islandContainer.showToolsMenu();
    }

    function focusToolsMenu() {
        if (toolsMenuLoader.item && toolsMenuLoader.item.grabKeyboardFocus)
            toolsMenuLoader.item.grabKeyboardFocus();
    }
    ...
    function focusCalculator() {
        if (calculatorLoader.item && calculatorLoader.item.grabKeyboardFocus)
            calculatorLoader.item.grabKeyboardFocus();
    }

    function toggleAgentVoiceWindow() {
        if (islandContainer.islandState === "agent_voice")
            islandContainer.smartRestoreState();
        else
            islandContainer.showAgentVoice();
    }

    function showAgentVoiceWindow() {
        islandContainer.showAgentVoice();
    }

    function toggleAgentTextWindow() {
        if (islandContainer.islandState === "agent_text")
            islandContainer.smartRestoreState();
        else {
            islandContainer.showAgentText();
            focusAgentText();
        }
    }

    function showAgentTextWindow() {
        islandContainer.showAgentText();
        focusAgentText();
    }

    function focusAgentText() {
        if (agentTextLoader.item && agentTextLoader.item.grabKeyboardFocus)
            agentTextLoader.item.grabKeyboardFocus();
    }
    ...
    FocusScope {
        ...
        readonly property bool toolsMenuLayerVisible: !root.overviewVisible && islandState === "tools_menu"
        ...
        readonly property bool agentVoiceLayerVisible: !root.overviewVisible && islandState === "agent_voice"
        readonly property bool agentTextLayerVisible: !root.overviewVisible && islandState === "agent_text"
        ...
        function showAgentVoice() {
            cancelSideSwipeSettle();
            abortSideTransientMode();
            clearTransientCapsule();
            islandState = "agent_voice";
            mainCapsule.displayedWidth = mainCapsule.baseTargetWidth;
            stopAutoHideTimer();
        }

        function showAgentText() {
            cancelSideSwipeSettle();
            abortSideTransientMode();
            clearTransientCapsule();
            islandState = "agent_text";
            mainCapsule.displayedWidth = mainCapsule.baseTargetWidth;
            stopAutoHideTimer();
        }
        ...
        Rectangle {
            ...
            readonly property real baseTargetWidth: {
                ...
                case "clipboard":
                    return 560;
                case "calculator":
                    return (calculatorLoader.item && calculatorLoader.item.hasGraph) ? 640 : 420;
                case "agent_voice":
                    return 140;
                case "agent_text":
                    if (!agentTextLoader.item) return 420;
                    return agentTextLoader.item.preferredWidth;
                case "notification":
            ...
            readonly property real targetHeight: {
                ...
                case "clipboard":
                    return 340;
                case "calculator":
                    if (!calculatorLoader.item) return 110;
                    if (calculatorLoader.item.hasGraph) return 440;
                    return calculatorLoader.item.hasResult ? 190 : 110;
                case "agent_voice":
                    return 140;
                case "agent_text":
                    if (!agentTextLoader.item) return 64;
                    return agentTextLoader.item.preferredHeight;
                case "notification":
            ...
            // Pitch-black to transparent linear gradient background (top to bottom)
            gradient: Gradient {
            ...
            Loader {
                ...
                sourceComponent: Component {
                    SwipeLyricsLayer {
                        ...
                        cavaLevels: islandContainer.cavaLevels
                        isPlaying: islandContainer.isPlaying
                        timeText: timeObj.currentTime
            ...
            Loader {
                id: toolsMenuLoader
                anchors.fill: parent
                active: islandContainer.toolsMenuLayerVisible
                asynchronous: false
                visible: islandContainer.toolsMenuLayerVisible
                onLoaded: root.focusToolsMenu()

                sourceComponent: Component {
                    ToolsMenuLayer {
                        iconFontFamily: root.iconFontFamily
                        textFontFamily: root.textFontFamily
                        showCondition: islandContainer.toolsMenuLayerVisible
                        onCloseRequested: islandContainer.smartRestoreState()
                    }
                }
            }
            ...
            Loader {
                id: calculatorLoader
                anchors.fill: parent
                active: islandContainer.calculatorLayerVisible
                asynchronous: false
                visible: islandContainer.calculatorLayerVisible
                onLoaded: root.focusCalculator()
            ...
            Loader {
                id: agentVoiceLoader
                anchors.fill: parent
                active: islandContainer.agentVoiceLayerVisible
                asynchronous: false
                visible: islandContainer.agentVoiceLayerVisible

                sourceComponent: Component {
                    AgentVoiceLayer {
                        iconFontFamily: root.iconFontFamily
                        textFontFamily: root.textFontFamily
                        showCondition: islandContainer.agentVoiceLayerVisible
                        onCloseRequested: islandContainer.smartRestoreState()
                    }
                }
            }
            ...
            Loader {
                id: agentTextLoader
                anchors.fill: parent
                active: islandContainer.agentTextLayerVisible
                asynchronous: false
                visible: islandContainer.agentTextLayerVisible
                onLoaded: root.focusAgentText()

                sourceComponent: Component {
                    AgentTextLayer {
                        iconFontFamily: root.iconFontFamily
                        textFontFamily: root.textFontFamily
                        showCondition: islandContainer.agentTextLayerVisible
                        onCloseRequested: islandContainer.smartRestoreState()
                    }
                }
            }
